...
This module classifies supplied historical evidence only. It performs no live
acquisition, persistence, recommendation, canonical-selection, or production
operation.
"""
...
def classify_record(record: Mapping[str, object]) -> AuditRecord:
    """Classify one already-captured historical NV/PARTIAL record.

    Precedence is causal: orchestration failure, source unavailable, missing
    required data, then genuine insufficiency. A record cannot be labelled
    genuine insufficiency when an avoidable upstream failure is evidenced.
    """
    ...
    if not orchestration_ok:
        return AuditRecord(assessment_id, FailureClass.ORCHESTRATION_FAILURE,
                           ("historical orchestration did not complete successfully",))
    if not source_available:
        return AuditRecord(assessment_id, FailureClass.SOURCE_UNAVAILABLE,
                           ("required source was unavailable at assessment time",))
    if not required_data_complete:
        return AuditRecord(assessment_id, FailureClass.MISSING_DATA,
                           ("required source existed but required data was missing",))
    if not evidence_sufficient:
        return AuditRecord(assessment_id, FailureClass.GENUINE_INSUFFICIENCY,
                           ("available complete evidence was genuinely insufficient",))
